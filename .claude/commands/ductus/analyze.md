...
<!-- audit:ignore-promotion -->
17. Record the run (host responsibility): invoke `write-analysis` with the feature, the current UTC timestamp, the HEAD sha, and the run's tier counts — `hard-fail`, `blocking-findings`, `advisory`, and `unexamined` (the size of the Informational tier's unexamined-target set). It writes the spec's `analyze:` frontmatter block, the durable record that this command ran and what it found. **Always**, including on a clean run and on a run whose scope was empty: the record's whole purpose is that its *absence* means something, so a run that declines to write one is indistinguishable from a run that never happened.
