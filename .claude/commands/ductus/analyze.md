...
- Never modify an artifact under audit. Three writes are in scope and no others: the inbox capture of surviving findings (step 16), the spec's `analyze:` frontmatter block (step 17), and — only with `--fix` — the revert of a drifted `done` spec from `done` to `in-progress` via `set-status` on either trigger — review-state drift (see Review state drift below) or unresolved scenario open questions (see Scenario open questions below). Without `--fix`, no file is written.
...
- **No grandfather rule**, unlike Review state drift below. An absent `review:` block genuinely marks a spec as predating `/ductus:review`; an unresolved scenario question is a present-tense defect whenever it arrived, and exempting it would preserve exactly the state this check exists to surface.
...
- `review.last-run` is set to a non-null timestamp. If the `review:` block is **present** but `last-run` is missing or `null`, report `Review drift: done spec missing review — run /ductus:review` (**blocking**)
- `review.blocking` is `false`. If `true`, report `Review drift: done spec has unresolved MUST violations — see review.md` (**blocking**)
- `review.should-violations` is `0`. If greater, report `review drift: done spec has {n} outstanding SHOULD violation(s) — fix each, or move it under review.md's Waived findings with its rationale, then re-run the review so the count states what is outstanding` (**blocking**). §implement-phase gives a SHOULD exactly two dispositions — fixed, or moved under `review.md`'s Waived findings with its rationale — and both drop the count, so a non-zero count at `done` means neither happened. **Review block agreement** cannot catch it: one `/ductus:review` run writes both the block and `review.md`, so a fix applied *during* a pass that never re-runs leaves the two consistently stale and that check clean.

**Grandfather rule.** A `done` spec whose frontmatter has no `review:` block at all is treated as pre-`/ductus:review` and exempt from this check. The block is added by the spec template (so every newly-scaffolded spec ships with it) and by `/ductus:review` on first run; its absence on a done spec means the spec reached done before `/ductus:review` existed. Adopters who want retroactive review run `/ductus:review` against the spec to populate the block, after which the spec is subject to the drift check on every subsequent analyze.

Specs not at `status: done` are silently exempt — the `review:` block is populated lazily on first `/ductus:review` run, so its absence on `draft` / `clarified` / `planned` / `in-progress` specs is normal.
