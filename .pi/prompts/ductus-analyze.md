---
description: Audit artifacts against each other — spec, plan, tasks, scenarios, frontmatter, dependencies, rule IDs. Never modifies an artifact it audits; records the run and its findings, and --fix reverts a done spec drifted by review state or unresolved scenario questions.
...
Audit a feature's spec, plan, tasks, and data model for consistency. Reports issues without modifying the artifacts it audits. Use this to catch problems before the next pipeline gate fires. It writes exactly two things **about itself**: each surviving finding to the inbox (step 16), and `analysis.md` — the durable record that this run happened, in that file's own frontmatter (step 17). Neither is an exception to the read-only contract — they are what that contract is about. The line is between the *subject* and the *observation*: analyze never mutates an artifact it audits, and `--fix` remains the only path that does. Recording that the audit ran is what `/ductus:review` has always done through `review.md`, and precisely why that gate was enforceable while this one was not: until the record existed, a spec that had passed both gates and one that had passed only the review were byte-identical on disk. The `--fix` flag, which reverts a status-`done` spec back to `in-progress` on either of two triggers: a review record drifted to blocking (see Review state drift), or unresolved open questions carried by the spec's scenarios (see Scenario open questions). Those reverts are the only writes this command performs, and neither is silent — each emits a notice naming the spec and what drifted.

Renamed from `/validate` in spec 023 to align with the emerging spec-driven-development standard (GitHub Spec Kit uses `/analyze` for the same artifact-vs-artifact audit role). Complementary to `/ductus:review`, which audits **code** against rules.
...
If `--all` is not present, use the feature identifier if provided, otherwise fall back to the session target from `.ductus/session.toml`. If no target can be resolved, stop and tell the user to run `/ductus:target` first or use `--all`.

## Scope Boundaries

- Never modify an artifact under audit. Three writes are in scope and no others: the inbox capture of surviving findings (step 16), the feature's `analysis.md` (step 17), and — only with `--fix` — the revert of a drifted `done` spec from `done` to `in-progress` via `set-status` on either trigger — review-state drift (see Review state drift below) or unresolved scenario open questions (see Scenario open questions below). Without `--fix`, no file is written.
- Read only files within the target feature's directory, the cross-spec files needed for reference checks (`specs/system.md`, `specs/events.md`, `specs/errors.md`, dependency spec files), and the project's installed command-source frontmatter for the project-level consistency section below (`.claude/commands/ductus/*.md` frontmatter only, plus `.claude/commands/ductus.md` frontmatter for the bootstrap installer **if that file exists**). May invoke `scripts/gen-help-tables.sh --dry-run` to surface generator drift, only when that script exists in the project (it is a ductus-repo-only generator and is absent from adopters), and `derive-dependencies` in its report-only default to surface frontmatter drift. Do NOT read source code or test files.
...
- Reference: §spec-requirements, §grounding, §plan-phase, §tasks-phase, §readiness-check, §scenarios, §cross-spec-impact, §text-first-artifacts, §markdown-standards, §drift-prevention (constitution loaded by `/ductus:target` — do not re-read). See [030 — Cross-Service References](https://github.com/stonean/ductus/blob/main/specs/030-cross-service-references/spec.md) for the reference semantics surfaced here.
...
2. Invoke `validate-frontmatter` against the spec path to check that the YAML block parses and that the required fields (status, dependencies) are present with valid values. Each finding carries the severity §text-first-artifacts (Validation Severity) assigns to **that** condition, so render each finding in the tier it names — never map the family to one fixed tier. The distinction is load-bearing in both directions: a malformed block or a missing or invalid required field is Hard fail, while a residual `review:`/`analyze:` record block is **Blocking** precisely because the spec file itself parses. A family flattened up to hard-fail overstates the second; one flattened down to blocking understates the first. The rest of the procedure still runs to surface every issue in a single pass.
...
5. Invoke `check-rule-ids` against the spec path with the project's rule files. Cited rule IDs that are missing are blocking; cited rule IDs marked deprecated are advisory. **Read `examined` before acting on `missing`.** It reports how many rule files were read, and `missing` is only meaningful against it: the known-ID set is built from the `rule-files` argument, so an `examined` of **0** makes every citation `missing` and means *nothing was checked against*, not *these IDs do not exist*. A blocking finding raised from that state is against a correct spec, and "fixing" it deletes a valid citation. Pass the files the rule-file discovery step selected, and treat `examined: 0` on a project that has rule files as a caller defect rather than a result.
...
<!-- audit:ignore-promotion -->
16. Capture the findings (host responsibility): call the append-inbox primitive once per surviving finding to record it in `{specs-root}/inbox.md` before anything is rendered, so an audit's results outlive the session that ran it (§brownfield-inbox Automatic issue capture). A finding survives when it is still live at the end of the run: `--fix` resolved findings are not captured, and informational entries are not findings — the unexamined-target `skipped` list and cross-service reference unknowns report what could not be examined, not a defect to route. Each bullet takes the auto-capture form `{category}: {family} — {message} — {path} (captured during /ductus:analyze)`, and each append passes a **dedup prefix** of `{category}: {family} — {message}` — a true prefix of that bullet — so re-running the audit against an unchanged repo appends nothing. The message belongs in the key: a finding's `path` is the *citing* artifact, not the missing subject, so keying without the message merges every finding one `spec.md` produces and drops all but the first.

<!-- audit:ignore-promotion -->
17. Record the run (host responsibility): invoke `write-analysis` with the feature, the current UTC timestamp, the HEAD sha, the run's tier counts — `hard-fail`, `blocking-findings`, `advisory`, and `unexamined` (the size of the Informational tier's unexamined-target set) — and the findings captured at step 16, as `captured-issues`. It writes `specs/{feature}/analysis.md` — the run's record in that file's frontmatter, plus the report body's fixed skeleton — the durable record that this command ran and what it found. `spec.md` is not written: the record lives in the artifact this command owns (spec 057). **Always**, including on a clean run and on a run whose scope was empty: the record's whole purpose is that its *absence* means something, so a run that declines to write one is indistinguishable from a run that never happened.

    This is what `/ductus:implement`'s pre-done gate reads to hold a spec out of `done` until this command has actually run, and it exists because it did not. The pipeline is `/ductus:review → /ductus:analyze → done`, and only the first half left a trace: that gate read the review record, Family 19 checked its freshness, a reconciliation family held it against `review.md`, and analyze wrote nothing at all. A spec that had passed both gates and one that had passed only the review were byte-identical on disk, so nothing could enforce the second — the only thing holding it was whoever remembered, which is the diligence dependency §design-principles rejects. On 2026-09-05 two specs were advanced to `done` on the review gate alone and one was published to crates.io, irreversibly, before anything noticed, because nothing could.

    **`unexamined` is not optional and not decorative.** A clean analyze is two different states — every target examined and clean, or some target unexaminable and the rest clean — and the Unexamined targets section below says so in as many words. A record carrying only the finding counts would collapse exactly that distinction into the reassuring reading, inside the artifact a later gate trusts, which is the worst possible place for `QUAL-CLAIM-001`. Write the count even when it is zero; a zero that was computed and a field that was never written are not the same claim.

    **`captured-issues` is the pair to `advisory`, and it exists because the two can silently disagree.** The capture step above records each surviving finding to the inbox and this step records how many the run *produced*, but the appends are separate calls — so a run that recorded `advisory: 5` and appended nothing was byte-identical on disk to one that captured all five, and nothing could tell them apart. The review side once had a reconciliation check holding `review.md` against the spec's `review:` block; spec 057 gave each record one home and retired it, and gave analyze a report artifact of its own, so this field rather than a cross-artifact comparison is what makes the divergence legible. Passing what step 16 actually captured is what populates it. **The two are not required to match, and forcing them would be wrong:** the `dedup-prefix` guard legitimately suppresses a re-append, so a correct re-run against an unchanged repo captures fewer than it found — `advisory: 5` beside `captured-issues: 0` is the *expected* second run. What the field buys is that both numbers are stated, so the case where they disagree for a bad reason is visible rather than invisible.

    `blocking` is derived by the primitive from `hard-fail` and `blocking-findings` — advisory findings are recorded and never gate. That asymmetry with the review record is deliberate: an outstanding SHOULD blocks `done` at the review gate because §implement-phase says advisory is not ignorable there, while analyze's advisory tier is made of checks introduced advisory **with their own published promotion criteria** (grounding, Applicable-Rules citations, decision drift), and gating on them here would promote every one of them at once, past the criteria each declares.

    Order matters: this runs **after** the capture step and **before** the render, for the same reason capture precedes render — a run interrupted between recording and reporting must leave the record, not the report. On the markdown-only path, write `analysis.md` by hand with the same field set and the same section skeleton. A spec whose frontmatter does not parse gets **no** record: the analysis would have hard-failed on it, and writing a clean record into it inverts the whole mechanism.

<!-- audit:ignore-promotion -->
18. Render the report (host responsibility): list hard-fail and blocking findings first, advisory findings next, then informational. For each finding, include what failed, what was expected, what was found, and a suggested fix. With `--fix` set, additionally revert any status-done spec either check found drifted — a review record drifted to blocking, or unresolved open questions in the spec's scenarios — through the guarded set-status revert (`from: done`, `to: in-progress`), detailed in the Review state drift and Scenario open questions sections in the markdown-only reference below. Each revert emits its own non-silent notice.
...
- The frontmatter parses as valid YAML.
- Either the `section` field (new schema) or the legacy `spec-ref` field is present and non-empty. New scenarios written by `/ductus:amend` use `section`; pre-017 scenarios written before `section` existed may still carry `spec-ref`. Either field satisfies the check.

Reference: the schema is canonically declared in `framework/constitution.md` §text-first-artifacts.

### Spec integrity (blocking)

- Acceptance criteria section exists at every status (§spec-requirements). It carries **at least one checkbox item at `clarified` or later** — the minimum belongs to the clarify gate, not to `draft`. A brownfield sketch spec legitimately has none: §brownfield-process says sparse acceptance criteria are expected and valid, `/ductus:specify` tells authors to leave the section with a placeholder comment when no criteria are known yet, and `framework/templates/spec/spec.md` scopes the requirement the same way — *at least one concrete, testable criterion is required before `/ductus:clarify` will advance the spec*. Flagging a zero-criterion `draft` would gate the brownfield entry path on day one, and it is the one place the two execution paths disagreed: `check-artifacts` implements no minimum-criterion family among its nine, so the runtime already answered this way while this reference did not.
- No placeholder or empty acceptance criteria
- Open questions consistent with status — the **spec body's** `## Open Questions` (`clarified` or later must have none). Scenario open questions are a separate signal with their own check (see Scenario open questions below) and never count here. When this check fails — a spec at `clarified` / `planned` / `in-progress` with one or more open questions in the body — the spec is in the recovery state defined by spec 014. Suggested fix: run `/ductus:clarify` (its recovery path will revert status to `draft` and walk the questions), or `/ductus:amend` on a fresh question (which performs the back-edge automatically).
...
Enforces `.ductus/constitution.md` §grounding against the spec and plan bodies: a factual claim about the **existing system** must be grounded — either cited to a primary source or marked as an assumption — rather than asserted from conjecture. `/ductus:analyze` checks the *form* of grounding (is the claim sourced or hedged), not its truth: confirming a claim against the code would require reading source, which is out of this command's scope (see Scope Boundaries). The truth check is the agent's job at authoring time (§grounding) and `/ductus:review`'s job against code.
...
- Every scenario file in `scenarios/` has a corresponding task in `tasks.md` **only while that task is still pending**. `tasks.md` is an ephemeral tracking artifact (§tasks-phase) that `/ductus:prune` reduces once work is complete, so a *missing* scenario task is a finding only when the scenario is unimplemented; do NOT flag a scenario whose task was completed and pruned, and do NOT flag any scenario under a `done` spec (its tasks may have been pruned, or the file reset to template state). The durable record of an implemented scenario is the scenario file, the code, and git history — not a retained checkbox.
...
- **No grandfather rule**, unlike Review state drift below. An absent `review.md` genuinely marks a spec as predating `/ductus:review`; an unresolved scenario question is a present-tense defect whenever it arrived, and exempting it would preserve exactly the state this check exists to surface.
...
Suggested fix per finding, offering both exits: resolve each question via `/ductus:target {feature}/<scenario>` then `/ductus:clarify` (scenario-targeted), **or** — when the question is deferred rather than undecided — move it to the scenario's `## Resolved Questions` with the condition that will settle it.

When `--fix` is set, this check additionally reverts affected specs from `done` to `in-progress` — via `set-status` (`from: done`, `to: in-progress`) on the runtime path, a direct frontmatter edit on the markdown-only path — and emits a one-line notice for each (`reverted: specs/{feature}/{file} from done to in-progress — unresolved scenario questions in {scenarios}`). The revert is never silent. Specs below `done` are not reverted: their finding is advisory, so there is no drifted state to correct. See [046 — Scenario open-question visibility](https://github.com/stonean/ductus/blob/main/specs/046-scenario-open-question-visibility/spec.md).
...
- **A criterion asserting a path was never created is exempted the same way.** One construction cannot be written as a phrase: the negator is separated from its verb by the noun it negates — ``no nested `server/.git/` repository was created`` — so no fixed phrase spans them, and the phrase that would, `was created`, exempts positive delivery claims too. Blinding the family is worse than the false positive it fixes, so this group is a clause-scoped predicate instead: a criterion is exempted whole when one of its clauses carries a negator (`no`, `not`, `never`, `without`) **before** a creation verb (`created`, `added`, `introduced`). Both lists are closed and framework-fixed, like the phrases above. Clauses split on `;`, `,` and a period *followed by a space* — never the bare character, which would cut `server/.git/` between the negator and the verb. The ordering is load-bearing in both directions: ``no `X` was created`` is a negated creation, while ``X was created and not modified since`` is a delivery claim carrying a trailing qualifier and still flags.
...
Suggested fix per finding: correct the criterion to name the current path, or — when the deletion means the criterion is no longer satisfied — uncheck it and route the work through `/ductus:amend`.

### Unexamined targets (informational)

Both checks above read *targets* that may be unreadable: a link resolving to a file that no longer exists, a target whose frontmatter will not parse, a target carrying no state the tell's class can be evaluated against. None of these produces a finding — an unknown is never escalated into a defect, matching the `status-unreadable` precedent in **Cross-service references** below — **with one exception**: `artifact-unreadable` on a spec at `status: done` is a **blocking** finding rather than a skipped target.

The exception holds where the rule's reasoning does not. That rule is about *other* files — another spec's frontmatter, an upstream service's state — where the defect belongs to someone else and blocking this spec would punish the wrong artifact. `artifact-unreadable` names the spec's **own** artifact, in its own directory, that its own analysis could not read: not an unknown about someone else, but the analysis unable to examine its own subject. The concrete case is **Scenario open questions**, which is blocking at `done` — an unreadable scenario contributes no questions and, as a skip, no finding, so a scenario carrying unresolved questions that will not parse passed the gate built to catch exactly that. Below `done` it stays a skipped target: the questions check is advisory there, the spec is in flight, and an unreadable artifact mid-work is a state to report rather than a gate to fail.
...
- **An unlabelled criterion, once the spec carries `next-criterion` at all.** The gate is the field's presence, not a date: a spec with no counter has never been through the pass, and §text-first-artifacts defines that absence as "no labels assigned yet" rather than a defect. The corpus backfill is what makes the check universal — deliberately, rather than a per-spec grandfather date, which is the exemption state [046](https://github.com/stonean/ductus/blob/main/specs/046-scenario-open-question-visibility/spec.md) refused for scenario questions on the grounds that a sanctioned hiding place is worse than the gap it papers over.
...
**Severity:** advisory. The pre-commit hook runs the pass on every commit, so the two assignable states — an unlabelled criterion, a counter that has fallen behind — are resolved by the next commit rather than owed to a gate; the two that are not assignable are rare hand-edit defects. See [013 — Text-First Artifacts](https://github.com/stonean/ductus/blob/main/specs/013-text-first-artifacts/scenarios/criterion-identifiers.md).
...
- **`unregistered`** (informational, not a finding) — the reference's repo matches no `[services]` entry. A plain navigational link; status was not attempted, so nothing is broken. Surface it with a pointer to `/ductus:link` to register the service.
...
The load-bearing line is **provably broken** (a finding) versus **can't check** (an informational unknown): a broken link never hides behind a benign unknown, and an unknown is never escalated to a defect. This classification matches the `resolve-references` primitive and the `/ductus:status` readout exactly — the three surfaces share one contract and none wraps another.

### Review state drift (blocking)

For each spec at `status: done`, read the review record from its `review.md` frontmatter:

- `last-run` is set to a non-null timestamp. If `review.md` is **present** but `last-run` is missing or `null`, report `Review drift: done spec missing review — run /ductus:review` (**blocking**). A `review.md` that exists but carries no parseable record is **undeterminable**, which is a hard fail rather than a never-run — the two must never render alike
- `blocking` is `false`. If `true`, report `Review drift: done spec has unresolved MUST violations — see review.md` (**blocking**)
- `should-violations` is `0`. If greater, report `review drift: done spec has {n} outstanding SHOULD violation(s) — fix each, or move it under review.md's Waived findings with its rationale, then re-run the review so the count states what is outstanding` (**blocking**). §implement-phase gives a SHOULD exactly two dispositions — fixed, or moved under `review.md`'s Waived findings with its rationale — and both drop the count, so a non-zero count at `done` means neither happened. Nothing else catches it: the reconciliation family that once held the spec's `review:` block against `review.md` was retired with that second home (spec 057), and it could not have caught this anyway — one `/ductus:review` run wrote both sides, so a fix applied *during* a pass that never re-runs left them consistently stale and that check clean.

**Grandfather rule.** A `done` spec with **no `review.md` at all** is treated as pre-`/ductus:review` and exempt from this check. The absent artifact is the never-run state (§text-first-artifacts): `/ductus:review` writes the file on its first run, so its absence on a done spec means the spec reached done before `/ductus:review` existed. Adopters who want retroactive review run `/ductus:review` against the spec, after which it is subject to the drift check on every subsequent analyze. Note what the exemption is keyed on — the artifact, never a block in `spec.md`. Keyed on the block it would exempt every spec in a migrated corpus, which is the vacuity spec 057 removed from the shipped CI template.

Specs not at `status: done` are silently exempt — `review.md` is written lazily on first `/ductus:review` run, so its absence on `draft` / `clarified` / `planned` / `in-progress` specs is normal.

**`--fix` reverts; it never edits the count.** The count is the review's to write, and rewriting it here would erase the finding rather than resolve it. When `--fix` is set, this check additionally reverts affected specs from `done` to `in-progress` — via `set-status` (`from: done`, `to: in-progress`) on the runtime path, a direct frontmatter edit on the markdown-only path — and emits a one-line notice for each (`reverted: specs/{feature}/{file} from done to in-progress — re-run /ductus:review`). The revert is never silent; the notice is the point of the action. Re-running `/ductus:review` on each reverted spec is left to the operator — auto-running it during `--fix` is out of scope. The grandfather rule applies under `--fix` too: pre-feature `done` specs with no `review.md` are never reverted.

### Rules (blocking and advisory)

Rules are the cross-cutting tier of the framework's three-tier requirement model (see §rules in `.ductus/constitution.md`). Discover rule files by directory walk: list every `*.md` file in the project's rule-file directory and classify each by basename suffix per the closed-suffix policy declared in `.ductus/constitution.md` §rules — `*-backend.md`, `*-frontend.md`, `*-cross.md`, or unrecognized. `/ductus:analyze` loads **every** discovered file regardless of detected stack — and regardless of the project's `[rules] surfaces` setting (`ductus.md`, **Project Configuration**) — because citation verification spans surfaces: a backend project that cites `FE-XSS-001` in a scenario covering HTML output still needs that citation verified. `[rules] surfaces` scopes `/ductus:review` enforcement only (which surface's rules are checked against code); it never prunes the rule-file set `/ductus:analyze` loads for citation resolution.
...
These checks span the project's installed command set and constitution rather than the target feature. They catch drift in the framework files `ductus` ships, surfaced per the Drift Prevention principles in `.ductus/constitution.md` §drift-prevention. Run once per `/ductus:analyze` invocation regardless of which feature is targeted; with `--all`, run once before per-feature output.

Read inputs:

- `.ductus/constitution.md` (already loaded by `/ductus:target`)
- `.claude/commands/ductus/help.md`
- The full set of `.md` files in `.claude/commands/ductus/` (frontmatter only — do not read bodies for these checks)
...
```text
- [ ] {category}: {family} — {message} — {path} (captured during /ductus:analyze)
```
...
other item does — `/ductus:groom` routes it, or the work that resolves it
