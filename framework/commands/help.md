...
| `/{project}:analyze` | — | Audit artifacts against each other — spec, plan, tasks, scenarios, frontmatter, dependencies, rule IDs. Never modifies an artifact it audits; records the run and its findings, and --fix reverts a done spec drifted by review state or unresolved scenario questions. |
