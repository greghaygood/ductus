...
/{project}:review — 042-example-feature
  ...
  inbox       6 items outstanding, oldest 2026-05-19 — run /{project}:groom to route
  blocking: no
...
does not read the inbox, `/{project}:status` does not mention it,
...
and `/{project}:implement`'s `inbox-additions` — so an item older than the
...
```text
  inbox       6 items outstanding, oldest 2026-05-19 — run /{project}:groom to route
  inbox       6 items outstanding, age undeterminable — run /{project}:groom to route
  inbox       ✓ clean
...
`/{project}:status` was considered and rejected: it is a number the operator
