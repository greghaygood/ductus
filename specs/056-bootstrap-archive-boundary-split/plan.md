...
The whole change is `framework/` plus one non-published data file under `runtime/`. No runtime source changes, no version bump, no release tag. That is a conclusion from the consumer walk in the spec's §What the split does not reach, not an assumption, and task 5 re-earns it by running the families rather than re-reading the table.
