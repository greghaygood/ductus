...
next-criterion: 8
analyze:
  last-run: 2026-09-13T12:42:00Z
  analyzed-against: 382d31b5189d8eb7270780007acbcf07fb5aeadc
  hard-fail: 0
  blocking-findings: 0
  advisory: 0
  unexamined: 0
  analyzed-digest:
    plan.md: 7918ec7d2a108dbdc4e61c438438189157923c93db7f0dbabe9323339c43a547
    review.md: d0ba04e656241e2f10698786d8f89ea9ba2d444d80c0ef1b821bef1eee9a10bb
    spec.md: e2eeee5455e27a92eff9dd607028e6d9cf3bfc6b59ee133c15ef185b06f351de
    tasks.md: cc59654d1b058006460bef0fab0610ff2f7193c1b3eaa3ad198355af89887f6b
  blocking: false
---
...
This spec closes the asymmetry: a backend performance rule set citable by ID (`BE-{CATEGORY}-{NNN}`) that `/ductus:analyze` checks against specs and plans and `/ductus:review` checks against code.
...
- [x] AC1: `framework/rules/performance-backend.md` exists, ends in the `-backend.md` suffix, and follows the canonical rule schema (`### {ID}` headings; Statement / Rationale / Verification; RFC 2119 language) per [008-security-rules](../008-security-rules/spec.md)'s data-model.
- [x] AC2: Every rule ID uses the `BE-{CATEGORY}-{NNN}` format with a performance category (`QUERY`/`CACHE`/`POOL`/`PAYLOAD`/`ASYNC`) disjoint from the `security-backend.md` and `api-backend.md` category sets; `scripts/lint-rule-ids.sh` passes.
...
- **Verification style for N+1 / index rules.** Resolved: **design-time commitments** targeting specs/plans (e.g., "any plan that introduces a list-rendering endpoint MUST state its query strategy and the indexes it relies on"), not code-pattern detection — consistent with `/ductus:analyze` auditing artifacts rather than source.
