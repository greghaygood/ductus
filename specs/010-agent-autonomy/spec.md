...
The stuck-detection *behavior* is worth adopting using existing signals: `/ductus:implement` instructions gain a step that reads `git log` for the affected paths and the current `tasks.md` checkbox state to detect when a task has been touched across N invocations without flipping to `[x]`. When detected, surface to the user and suggest decomposition. No new file, no schema, no git noise.
...
- [x] AC5: Changes respect command file parity (commands/ and .claude/commands/ductus/)
- [x] AC6: Changes respect ductus file parity (ductus/ variants stay in sync)
