...
- [x] AC11: The constitution's §spec-phase directory-layout block carries a single one-line note that the spec-root name is configurable via `[paths] specs-root` (default `specs`), with a back-pointer from the `.ductus/config.toml` schema docs; no other prose is parameterized.
