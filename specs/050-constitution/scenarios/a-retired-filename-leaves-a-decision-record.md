...
**A ticked acceptance criterion is swept like any other reference.** The requirement a criterion states is unchanged when only the file's name moved — `.ductus/config.toml` accepts the key `.govern.toml` accepted, and the criterion is as true after the substitution as before — so naming the file correctly asserts nothing new. Annotation is for a criterion whose **behavior** was superseded, which is a different thing: 022's AC6, AC9, AC12, AC16 and AC17 annotate retired *behavior*, and there rewriting the text would record a fiction. Annotating a rename is the wrong instrument twice over: it says at length what a substitution says exactly, and it preserves the retired name in the corpus permanently — the residue this rule exists to remove. [017](../../017-derive-dont-ask/spec.md) and [027](../../027-bootstrap-migration-registry/spec.md) annotated theirs before this was settled; those annotations are removed rather than swept, since after the substitution they would read *"`.ductus/config.toml` no longer exists"*.
...
- **A spec carrying a stale annotation from an earlier pass.** Removing it rewords the line, so that file is judged structural and the spec reopens — correctly, since a removed annotation is a factual correction rather than a substitution. Committing it apart from the sweep does not rescue it: the staleness window is `reviewed-against..HEAD`, not a single commit, so both edits land inside it.
