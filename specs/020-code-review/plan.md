...
# 020 — `/ductus:review` Plan

Implements [020 — `/ductus:review` code review command with blocking gate](spec.md).
...
`/ductus:review` is a slash-command file. The agent reads it on invocation and follows the instructions — there is no Go/TypeScript/Python module to write. This matches every other `/ductus:*` command and keeps the framework's "text-first artifacts" invariant intact. Trade-off: the agent's review quality is the agent's quality; we cannot bolt on a deterministic linter without changing the framework's shape. Acceptable because the existing security rules in `framework/rules/` are already authoritative natural-language rules read by the agent, not regex patterns.
...
| `framework/templates/ci/adopter-generators.yml` | Edit | Add a step that fails when any `done` spec has `review.blocking: true` or missing `review.last-run` (predicate replaced by 057 — see the CI-template decision above) |
...
- Existing `done` specs in adopter projects (pre-`/ductus:review`) carry no review record at all — no `review:` block as planned here, no `review.md` since 057. The first `/ductus:analyze` run after adoption will flag them. This is the intended behavior — adopters re-review or waive on adoption. **What changed with the relocation is that this population is now *bounded* rather than merely tolerated:** the grandfather rule keys on `review.md`'s absence, and the CI step holds the exempt set against a committed high-water mark, so the set can shrink but not grow.
- The CI gate parses the record's frontmatter in the workflow step rather than calling the runtime (in `python3` since the step was rewritten; awk, as planned here, before that). The `v2` this bullet anticipated has landed as [§runtime-boundary](../../framework/constitution.md#runtime-boundary) and the `ductus` binary — and **the workflow's earlier step already invokes it** (`.ductus/bin/ductus derive-dependencies --write`), so availability is not what keeps this step inline. The reason is that no primitive answers this step's question. `check-review-gate` evaluates the `in-progress → done` *transition* and returns early when a spec is already `done`, which is every spec this step examines; pointing the gate at them would report `passed` for the whole corpus. The two are complements — the gate holds a spec out of `done`, this step catches one that got there anyway — so §runtime-boundary principle 3's bar is not met here: there is no primitive this declines to call.
