...
Look up `introduced_in` per migration via `git log` against the commits that shipped each removal. Use `registry_introduction_version + 2 minor versions` for every `sunset_after` (where `registry_introduction_version` is the ductus release this spec lands in — pinned at release time).

- [x] Run `git log --diff-filter=D --name-only -- framework/templates/spec-and-plan.md framework/commands/spawn.md 'framework/skills/*'` (and similar) to find each migration's commit and corresponding ductus tag.
...
## 4. Rewrite `framework/bootstrap/ductus.md` Pre-run Migrations section
...
- [x] Run `/ductus:review` on the spec to surface any rule violations. (Ran 2026-05-22T02:32:17Z against HEAD `3e0053f`; loaded `security-backend.md`, `api-backend.md`, `configuration-cross.md`; skipped frontend rules (no frontend code in scope); tech-stack alignment skipped per `[review] tech-stack-verified = true`. Report at [review.md](review.md): 0 MUST / 0 SHOULD / 0 low-confidence across all five passes; `blocking: false`. Spec frontmatter updated with the review result.)
- [x] Resolve any MUST findings; record SHOULD findings as scenarios if deferred. (No findings to resolve; nothing to defer.)
- [x] Advance `spec.md` `status` from `in-progress` to `done` via the implement flow's done gate. (Flipped all 29 acceptance criteria to `[x]` first so the spec body reflects completed work, then ran `set-status` from `in-progress` → `done`. Final state: status=done, review.blocking=false, review.last-run set, all ACs checked. `/ductus:analyze` re-run post-flip confirms no Review-state-drift finding (done spec, review block populated and unblocked).)

- **Done when**: `/ductus:review` records 0 MUST violations with `blocking: false`, and `spec.md` is advanced to `done` via the done gate with all acceptance criteria checked.
...
## 13. Record that a migration applies once per repository, not once per contributor

- [x] Implement the behavior described in `scenarios/migrations-apply-once-per-repo.md`
- [x] State the scope in `specs/027-bootstrap-migration-registry/spec.md` §Adopter State: `[migrations].last_applied` is one marker in a committed file, so an entry is applied by the first contributor to run `/ductus` and skipped by everyone who pulls afterwards
- [x] State the rule that follows — a migration must not be the sole remedy for a defect in per-contributor state — and note that touching such state is fine when a skipped contributor self-heals through ordinary use
- [x] Name `session-file-consolidate` and `ductus-rename` as the self-healing precedent (a stale session file is superseded by the next `/{project}:target` under the newest-tier rule; the procedure exits silently when the legacy file is absent), so the rule is not misread as "never touch per-contributor state"
- [x] Restate the rule in `framework/migrations.toml`'s header comment, beside the existing note on what `target_paths` drives, where a migration author will actually meet it
- [x] Direct a cleanup that must reach every contributor to the command owning that file instead, naming `/ductus:configure`'s permission retirement (spec 023, `configure-retires-formerly-canonical-entries`) as the worked example and noting that `/ductus` never writes the full permission set anyway
- [x] Record that `target_paths` needs no change: `gitignore-marker-rename` already targets a file it edits rather than removes, and Family 10's no-stale-target-paths check constrains only `framework/`-prefixed entries
- [x] Note that `{config_dir}/commands/` is repo-shared despite living under `.claude/` (the shipped `.gitignore` excepts it), so `workflows-sunset` is not a counter-example

- **Done when**: 027's spec and `framework/migrations.toml`'s header both state that the registry applies an entry once per repository, and that a migration must not be the sole remedy for a per-contributor defect — with the self-healing precedent named so the rule is not over-read. The `target_paths` question raised when the permission retirement was first routed here is recorded as resolved by existing precedent rather than by a new field. `npx markdownlint-cli2` clean.
