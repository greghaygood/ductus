...
analyze:
  ...
  analyzed-digest:
    ...
    spec.md: 20b09c76ae62092cbb2bbf51b36203ccc33b01e46cb10941ff3524fa96e75745
...
Spec 017 shipped `framework/bootstrap/hooks/pre-commit` to adopters at `.githooks/pre-commit` with `update` strategy and a `# managed-by: ductus` sentinel on line 2. On every `/ductus` run, ductus overwrites that file in full whenever the sentinel is present (see [017-derive-dont-ask](../017-derive-dont-ask/spec.md) §Generators and Hooks; `framework/bootstrap/ductus.md` §Hook Installation, item 6).
...
| `.githooks/ductus-pre-commit` | ductus | `update` | The generator orchestration that ships with the framework (currently `gen-spec-deps.sh` + staging) |
...
```bash
#!/usr/bin/env bash
# Project pre-commit hook. Edit this file freely — /ductus does not overwrite it
# after the initial install. Add your project's pre-commit checks (lint, tests,
# format, etc.) above or below the ductus hook invocation below.

set -euo pipefail
cd "$(git rev-parse --show-toplevel)"

# Keeps ductus's generated artifacts (spec deps, etc.) in sync. /ductus updates
...
The detection ladder in `framework/bootstrap/ductus.md` §Hook Installation simplifies. The new ladder, replacing the current items 1–7:
...
2. **`core.hooksPath` points at any other path** — custom hooks dir. Skip wiring; report a warning with the manual integration snippet (see §Manual integration snippet below).
...
The previous "existing `.githooks/pre-commit` from a prior `/ductus` run, detected by sentinel" branch goes away — the outer file is no longer detected by sentinel because ductus doesn't own it. Migration of pre-existing ductus-installed hooks is handled separately (see §Migration below).
...
The path changes from `./.githooks/pre-commit` to `./.githooks/ductus-pre-commit`.
...
- **Executable bits on the new files.** `update` and `create` strategies write file content but do not chmod. After the manifest passes complete, `framework/bootstrap/ductus.md` §Hook Installation runs `chmod +x .githooks/pre-commit .githooks/ductus-pre-commit` once. Subsequent `/ductus` runs re-chmod (idempotent) — covers the case where an adopter's git config or worktree configuration dropped the executable bit.
...
- [x] AC2: A second shipped file (`framework/bootstrap/hooks/pre-commit`, replacing the file currently at that path) holds the initial content for the adopter-owned outer hook: invokes `./.githooks/ductus-pre-commit` and contains no `# managed-by: ductus` sentinel
...
- [x] AC5: `framework/bootstrap/ductus.md` §Hook Installation detection ladder is rewritten to the four-item form in §Design above; the "existing `.githooks/pre-commit` from a prior `/ductus` run, detected by sentinel" branch is removed
- [x] AC6: `framework/bootstrap/ductus.md` §Hook Installation manual integration snippet references `./.githooks/ductus-pre-commit`, not `./.githooks/pre-commit`
- [x] AC7: `framework/bootstrap/ductus.md` includes a migration subsection: when `.githooks/pre-commit` exists with the `# managed-by: ductus` sentinel on line 2 and no `.githooks/ductus-pre-commit` exists, rename the file and apply the manifest passes; report the migration in the post-scaffolding summary
...
- [x] AC9: A `/ductus` run on a fresh project (no existing hook) produces both files with executable bits set, sets `core.hooksPath .githooks`, and leaves only the inner file (`.githooks/ductus-pre-commit`) carrying the `# managed-by: ductus` sentinel
...
- [x] AC12: `framework/bootstrap/hooks/install.sh` is deleted; its install actions (`git config core.hooksPath .githooks` and `chmod +x` on the two hook files) are inlined into `framework/bootstrap/ductus.md` §Hook Installation; no other artifact references the deleted file
