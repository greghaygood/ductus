...
analyze:
  ...
  unexamined: 0
  analyzed-digest:
    plan.md: bc2164922db7fe1dca4b7c98abe7d3967aa801e333f20e515bfae069db19887b
    review.md: 4b5c8c14b8f0577a37cee7554d4b03860434f30d84306ab745efee1182de0624
    scenarios/applicable-rules-consistency-check.md: 654067cf29c934cccf9a88046d9aedf9b61728af9c5353037bb45ff4adf7d145
    spec.md: d3e855a8647d8cb4b317de0991050cddd02954c30f2d5d96f8072260b452ea2b
...
`ductus` already has working rule infrastructure. Spec 008 defined a strict rule format (ID, Statement, Rationale, Verification), wired per-rule verification into `/ductus:analyze`, hooked brownfield adoption into the inbox, and shipped two rule files (`security-backend.md`, `security-frontend.md`).

What 008 did *not* do was name **rules** as a general artifact tier. The constitution has §spec-phase, §scenarios, and §bug-handling, but no §rules. The bug decision tree routes a bug to a missing spec, an ambiguous spec, or a missing scenario — but never to a missing rule. The `/ductus:analyze` section enforcing rules is titled "Security rules." The spec template has no slot for citing applicable rules. The result: rules behave like a security feature rather than the artifact tier they actually are.
...
After this spec, every requirement in `ductus` lands at one of three levels of abstraction:
...
- [x] AC8: `/ductus:analyze --all` passes against the modified ductus repo (no new findings introduced by the spec/template/validate changes themselves).
...
- **Naming** — keep "rule." These are binding RFC-2119 statements that `/ductus:analyze` enforces, not guidelines. "Policy" reads softer and would understate the enforcement intent; it would also fork vocabulary already established by 008, the `framework/rules/` directory, and the constitution's "Secure" principle reference.
...
- **Validate alignment** — defer the stronger check (validate comparing triggered rules against listed Applicable Rules) out of 016's scope. The existing per-rule verification check (`analyze.md:130-134`) already catches rules whose triggers fire against an unaddressing spec, so 016 ships with the optional template section + existing reference check. The deferred work is captured in `specs/inbox.md` so it surfaces in a future `/ductus:groom` pass — keeps 016 focused without losing the follow-up signal.
