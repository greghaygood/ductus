...
- [x] In `framework/commands/groom.md` Context, replace the "a target is not required" note with: groom operates across all specs and does not *require* a target, but it now *sets* the session target when it routes an item to an existing spec (so a follow-on `/ductus:amend` / `/ductus:implement` needs no manual `/ductus:target`).
- [x] In Step 3 (spec edit), after the routing is confirmed, set `.ductus/session.toml` to the matched feature (`feature` + `path`), preserving any existing `cli-config-dir` (read first, carry forward; tempfile + rename), per the markdown-only session-write pattern in `specify.md` / `amend.md`.
...
- [x] State explicitly that Step 1 (rule item), Step 2 (new spec → `/ductus:specify`), and the Step 4 chore branch set **no** target. (New `### Setting the session target` subsection; Scope Boundaries updated to note the `.ductus/session.toml` write.)
...
- [x] Regenerate `.claude/commands/ductus/groom.md` (`gen-claude-commands.sh`) cleanly.
...
- [x] Run `/ductus:review` over the change set; resolve any MUST findings. (0 MUST / 0 SHOULD; 1 incidental issue captured to the inbox — groom not reopening done specs on scenario add — see `review.md`.)
