...
Additionally, the `analyze` command exists for on-demand auditing but its checks are not integrated into the pipeline. Validation should run automatically before every status transition, not rely on the user remembering to invoke it separately.
...
- During the completion phase, verify each acceptance criterion in the spec file (`spec.md`). Mark each passing criterion `- [x]` in the file.
...
Every pipeline command that proposes a status transition must run the relevant audit checks as a gate before asking the user to approve the transition. Failures block the transition.
...
The `/{project}:analyze` slash command remains available for on-demand auditing outside the pipeline.
