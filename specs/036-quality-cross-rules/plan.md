...
- **`### QUAL-STUB-001`** (MUST — per the clarify resolution):
  ...
  - **Rationale:** silent stubs ship indistinguishably from working code; the gap surfaces only under stress (the adopter rate-limiter passthrough that would have silently disabled rate limiting in production).
...
Add `| \`framework/rules/quality-cross.md\` | \`specs/rules/quality-cross.md\` |` to the `### ductus-owned shared files` table, slotted between `performance-frontend.md` and `security-backend.md` (AC #7). The §Shared Files "Rule-file surface filter" note was planned to have its count bumped from **six** to **seven** entries; implementation instead **removed** the hard-coded number, which had already drifted (it read six while seven files were listed) and which nothing machine-checks — eliminating the drift class rather than correcting one instance. Task 3 carries the decision; do not reintroduce a count here. The `-cross.md` suffix means 024's loader auto-selects it for every stack and 033's surface filter keeps it unconditionally — no change needed in either; the wiring is purely the manifest row + count.
...
- **008 / 017** define the rule schema and the `BE`/`FE` / `CFG` surfaces; 036 adds the `QUAL` surface in its own `data-model.md` and cites theirs. The constitution §rules governs the closed **filename-suffix** convention (unchanged) and does not enumerate rule-ID surfaces as a closed list, so §rules itself needs no edit. **One constitution edit is owed and was missed at delivery:** §drift-prevention's *Canonical sources* table carries a row for each of the `BE-`/`FE-` and `CFG-` surface registrations, and the section requires that a new kind of fact referenced from multiple documents name its canonical source there. `QUAL` is referenced from `framework/rules/quality-cross.md`, from `scripts/lint-rule-ids.sh`'s source-of-truth comment, and twice from the constitution itself, so the row belongs. Added 2026-09-13 during the examined/scope backfill, matching how `021` and `045` registered their own facts.
