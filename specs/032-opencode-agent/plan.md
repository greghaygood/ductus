...
and the `permission` set as two ductus-owned regions, where Claude splits them
...
### 3. Single committed root `opencode.json` — two ductus-owned regions
...
MCP-wiring file are the same target**, and ductus owns exactly two regions of it:

- `mcp.ductus` — `{ "type": "local", "command": ["ductus", "mcp"], "enabled": true }`
- `permission` — the bootstrap shell allows plus `"ductus*": "allow"`
...
surfaced instruction** (verified: ductus loads from the committed file, `opencode mcp
list` → `✓ ductus connected`). The §MCP wiring `write-file` branch currently writes
...
- ductus runtime → a single `"ductus*": "allow"` (one glob, like Antigravity's
  `mcp(ductus/*)`; verified accepted, no `mcp` permission key exists). Ordered
  **after** any broad `"*"` rule so it is not shadowed.
- `edit` → `"allow"` (ductus edits specs); `bash` → `{ "<allow patterns>":
...
Like Antigravity's constant `mcp(ductus/*)` line, OpenCode uses a single
`"ductus*": "allow"` glob (not per-tool enumeration), so its block is a constant
...
`merge-managed-block` content referenced in ductus.md and
...
carries the committed, team-shared ductus wiring (Decision 3). Like the Claude
...
Add an `opencode)` case: `dest=".opencode/command/ductus.md"`,
...
`ductus` command is a **verbatim** markdown file at `.opencode/command/ductus.md`
(invoked `/ductus`), placeholders kept literal — claude-style, not a wrapped skill.

### 9. Bootstrap-wide branches (mostly claude-style-like)

Because OpenCode's `ductus` installer is a verbatim markdown file (not a
...
- **No Rust change.** The State-B MCP write (Decision 4) and the configure merge
  (Decision 5) are host-side by the existing runtime boundary. `ductus`'s MCP tools
  operate on `specs/` independent of agent layout, so the runtime's value is
  available to OpenCode adopters regardless. Extending ductus `exec`'s `Host`
...
| File | Action | Purpose |
| --- | --- | --- |
| `framework/bootstrap/ductus.md` | Modify | `opencode` registry row + §Derived-values column; §MCP-registration row (write-file, project-committed); §MCP-wiring write-file `opencode` sub-case (root `opencode.json` `mcp` shape); §ductus-auto-wiring `"ductus*"` grant; §Per-Agent-Scaffolding `opencode` branch (verbatim namespaced commands, cleanup); §Permission-Setup seed; self-install / self-update / integrity / placeholder / dir-creation branches; §"Adding a new agent" note; `.gitignore` block adds `.opencode/` |
| `framework/bootstrap/configure/opencode.md` | Create | OpenCode permission set → root `opencode.json` `permission` (action map; `edit`/`bash` allows, denies, `"ductus*": "allow"`); prose-walk generic JSON merge; `generated:mcp-allow` markers |
| `scripts/gen-configure-mcp.sh` | Modify | Fourth splice target: emit constant `"ductus*": "allow"` block into `opencode.md`; keep the cross-agent in-sync invariant |
| `framework/templates/project/gitignore.md` | Modify | Add `.opencode/` to the managed block (root `opencode.json` stays tracked) |
| `install.sh` | Modify | `opencode)` arm: dest `.opencode/command/ductus.md`, seed root `opencode.json`, usage + error text |
...
| `scripts/tests/test-gen-configure-mcp.sh` | Modify | Assert the `opencode.md` `"ductus*"` block is emitted and stays in sync |
...
- **Runtime `exec` command resolution deferred** — OpenCode ships on the
  markdown-only path now; ductus MCP tools are unaffected. Tracked as a 022 follow-up.
...
derived-value formulas and the two ductus-owned `opencode.json` regions (`mcp`,
