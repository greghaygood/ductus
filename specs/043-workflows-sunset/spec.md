...
- **Spec 005 keeps `status: done` and gains a sunset note.** The lifecycle's five statuses gain no sixth value — `done` remains factually correct (the feature was delivered, criteria verified). A post-completion blockquote at the top of [005-workflows](../005-workflows/spec.md)'s body — the same idiom its existing 019 note uses — records that this spec removed the feature, that `framework/workflows/` and the `/ductus` recommendation flow no longer exist, and that the body below stands as the historical record of the feature as shipped. The note lands as part of this spec's removal sweep (mechanical-class under §spec-lifecycle; no back-edge).
