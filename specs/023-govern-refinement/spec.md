...
`ductus` exposes 14 user-visible slash commands across 5 categories. Two pairs do nearly the same thing, and one track concept exists because the model was wobbly on a question the framework already answers.
...
- Sweep every reference to `/{project}:validate`, `/ductus:validate`, and `validate.md` across `framework/`, `scripts/`, `.github/`, `README.md`, `AGENTS.md`, `docs/`, and `specs/README.md`. Replace with `/{project}:analyze`, `/ductus:analyze`, and `analyze.md` respectively. Audit count at spec time: 28 occurrences across 11 files.
...
- Done specs under `specs/NNN-*/` were initially left unswept, with the rename recorded once in `specs/README.md` under a "Past Renames" note so historical references stayed discoverable. The `living-specs` follow-on scenario superseded both halves: spec bodies are live artifacts swept with every other reference, `specs/README.md` §Past Renames is deleted, and rename history lives in git log.
...
### 5. `/configure` allow entries for ductus-owned state files

Adopters running pipeline commands are repeatedly prompted to confirm writes to files `ductus` itself owns — most visibly `.ductus/session.toml`, which `/target`, `/specify`, and `/amend`'s scenario branch all mutate as routine session-tracking steps. Asking the user to authorize a write to a file the framework owns is exactly the confirmation theater this refinement spec aims to remove.

`/configure` extends its canonical allow set with explicit per-path write entries for `ductus`-owned state. Initial scope: the session file. The principle generalizes — any future `ductus`-owned state file (e.g., additional sections of `.ductus/config.toml`) gets the same explicit allow treatment as it's introduced.

- **Claude**: `Edit(.ductus/session.toml)` and `Write(.ductus/session.toml)` added alongside the existing bare `Edit` and `Write` entries. The explicit path entries make intent clear and disambiguate from any agent-host heuristic that otherwise prompts on the session-file path. (The pre-0.10.0 form used `{cli-config-dir}/{project}-session.json`; consolidated onto the repo-root `.ductus/session.toml` in spec 022 task 40.)
...
- [x] AC7: `framework/commands/amend.md` classifies input as question or scenario, routes scenario inputs through the decision tree formerly in `/elaborate`, creates `scenarios/{slug}.md`, and appends a linked task to `tasks.md` on the scenario branch.
...
- [x] AC15: `framework/bootstrap/configure/auggie.md` includes an Auggie-format permission entry for every MCP tool listed in `framework/runtime-tools.txt`, added to the canonical permission set unconditionally.
- [x] AC16: The two configure sources draw from `framework/runtime-tools.txt` such that adding a new MCP tool to that file flows through to both agents on the next bootstrap (no manual edit per agent). The drift detection mechanism is a plan-phase choice (inline list with a generator that syncs from `runtime-tools.txt`, or runtime read at configure time).
...
- [x] AC18: The `ductus` release that ships with this spec exposes `create-scenario` and `append-task` as both CLI subcommands and MCP tools under the `gov-rt:` namespace. `framework/runtime-tools.txt` is updated to include both names.
- [x] AC19: `framework/commands/amend.md` (the rewritten version) invokes `create-scenario` and `append-task` on the scenario branch and falls back to host-side prose execution when the runtime is absent (per spec 022's markdown-only-path discipline).
- [x] AC20: `framework/commands/validate.md` is renamed to `framework/commands/analyze.md`; the H1 reads "# Analyze".
- [x] AC21: Across `framework/`, `scripts/`, `.github/`, `docs/`, `README.md`, `AGENTS.md`, and `specs/NNN-*/` (now uniformly live per the `living-specs` follow-on scenario), no file contains a current-usage reference to the old names `/validate`, `/ductus:validate`, `/{project}:validate`, or `validate.md`. All current-usage references have been swept to the new names (`/analyze`, `/ductus:analyze`, `/{project}:analyze`, `analyze.md`). Spec bodies of *introducing* specs (this one plus a few earlier ones tracked in the inbox) retain references to old names solely as historical-action descriptions in their own prose — these are accurate-as-of-then descriptions, not stale current-usage refs.
- [x] AC22: `scripts/gen-help-tables.sh` builds the pipeline table from `analyze.md`.
- [x] AC23: `specs/README.md` recorded `/validate → /analyze` under "Past Renames" at this spec's merge time so historical references in done specs were discoverable. The §Past Renames section was later deleted by the `living-specs` follow-on scenario, which removed the frozen-archaeology carve-out and brought done-spec bodies into the live-artifacts set; rename history now lives in git log.
- [x] AC24: `framework/commands/analyze.md`'s frontmatter `description:` reads exactly `Audit artifacts against each other — spec, plan, tasks, scenarios, frontmatter, dependencies, rule IDs. Read-only.`
...
- [x] AC34: The canonical `permissions.allow` set in `framework/bootstrap/configure/claude.md` no longer contains `Write(.ductus/session.toml)` or `Write(.ductus/config.toml)`. The `Edit(...)` entries for both paths are retained — they are what grants the access, for every file-editing tool — as are the bare tool-level `Edit` and `Write` entries under **File operations**, which name tools rather than paths. A rationale comment in the same section states the invariant and names the removed entries so a future editor does not re-derive it from a startup warning. `.claude/commands/ductus/configure.md` follows from the command generator, not a separate edit. See [`scenarios/configure-inert-write-path-entries.md`](scenarios/configure-inert-write-path-entries.md).
- [x] AC35: `/ductus:audit` Family 32a (`scripts/audit/permission-entry-shape.sh`) fails when a path-scoped `Write(...)` entry appears in either the canonical allow **or** deny section of a host that can express the shape. Deny is a subject here — unlike Family 29, whose defect grants too much and whose deny exclusion is therefore load-bearing — because an inert deny entry reads as a protected path and protects nothing. The bare `Edit` / `Write` tool entries are excluded by construction (the extraction requires a parenthesised argument), and the section's own rationale prose naming both removed entries is not reported as committing them. Only `claude.md` is examined; the three hosts with no path-scoped file-permission form are named on stderr. An empty extraction on a file that exists is a finding.
- [x] AC36: `/ductus:configure` carries a fourth numbered section listing the nine formerly-canonical `permissions.allow` entries it retires — the seven `Bash(git -C * <sub> *)` patterns dropped in 0.33.0 and the two `Write(.ductus/*.toml)` entries dropped by AC34 — grouped by the reason each was retired. The list is passed to `merge-permissions` as `revoke` on the runtime path; the markdown-only path performs the same exact-match splice before its own presence and dedup passes. The command's scope boundary records that retirement is confined to that explicit list, and the section records why the `/ductus` migration registry could not host this cleanup. See [`scenarios/configure-retires-formerly-canonical-entries.md`](scenarios/configure-retires-formerly-canonical-entries.md).
- [x] AC37: `merge-permissions` accepts a `revoke` list and removes every copy of each listed entry from `permissions.allow` by exact string match. Retirement is allow-side only **by construction** — the primitive has no deny-side counterpart, so an argument that could narrow the canonical deny set into holes does not exist. The revoke pass runs before dedup and canonical-presence, so a doubled retired entry is attributed wholly to `allow-revoked` rather than split with `allow-deduped`, and no removed entry is re-added. An entry present in both `allow` and `revoke` is rejected with `ConflictingRevoke` before any filesystem read, naming every overlap at once, because the two passes would otherwise fight and the `unchanged` short-circuit could never fire.
- [x] AC38: Run against a settings file carrying all nine retired entries, `/ductus:configure` removes them in one pass and reports `unchanged` on the second, preserving mtime. An adopter-authored near-miss of a retired pattern (`Bash(git -C ~/mine status *)`), the deny-side `Bash(git -C * rm *)`, sibling `permissions` fields, and unrelated top-level keys all survive untouched. A malformed settings file is reported and left alone rather than silently rewritten. Each removal is named in the command's closing report.
- [x] AC39: `/ductus:audit` Family 32b fails when an entry appears in both the retired list and the canonical allow set, so a maintainer's editing slip fails in this repo rather than in every adopter's `/ductus:configure` run. An absent retired section is a valid state and is skipped; a retired section that parses to zero entries is a finding.
- [x] AC40: The contrary decision recorded in [`scenarios/configure-permission-pattern-safety.md`](scenarios/configure-permission-pattern-safety.md) — that `/ductus:configure` does not retroactively strip an already-present unsafe entry — is annotated in place as superseded by AC36–AC38, naming the scenario that counters it. The bullet is annotated rather than deleted: it is the record of what shipped, and an inbound reference to it must not resolve to nothing.
...
- **Read-fallback lifetime for adopter `spec-and-plan.md` files.** Rip the fallback immediately. `ductus` is live-on-main per the README — there is no version a project can pin to, so a deprecation-cycle option has no load-bearing semantics. The read-side fallback exists in every pipeline command (`clarify`, `plan`, `implement`, `review`, `validate`, `target`, `status`, `amend`, `elaborate`); carrying it forward would undermine the simplicity goal of this spec. The adopter migration is trivial — `mv specs/NNN/spec-and-plan.md specs/NNN/spec.md` (the file format is identical; the lightweight track was a routing decision encoded in the filename, not a different schema). `done` specs with `spec-and-plan.md` stay on disk untouched — the framework does not rewrite an adopter's existing file, it only offers the rename. **Mitigation requirement (folded into AC):** `/ductus` bootstrap performs a one-pass migration check on every run, lists any `spec-and-plan.md` files it finds, and offers to rename them; the changelog accompanying this spec's release documents the rename. Adopters who upgrade blind without re-running `/ductus` see "Spec does not exist. Run `/ductus:specify` first." on the first pipeline command — the bootstrap check is what makes that case rare.
