...
The `settings_template` field in `framework/bootstrap/ductus.md`'s agent registry (lines 46–48) seeds the per-adopter `{config_dir}/settings.local.json` at first `/ductus` run and gets merged into existing files on subsequent runs (see Behavior steps 1–3 in the same doc). The template's `permissions.allow` array determines which Bash command patterns run *during bootstrap* without prompting the operator.

The Claude row's allow list covered `Bash(curl *)`, `Bash(ls *)`, `Bash(tar *)`, `Bash(mktemp *)`, plus several `Read(...)` entries for temp directories when this scenario was written. **The audit below has since landed**: the row now carries twenty `Bash(...)` patterns, including both confirmed gaps (`Bash(git status *)`, `Bash(awk *)`), and the six `Read(...)` temp-directory entries remain. `framework/bootstrap/ductus.md` §Agent Registry is the current list — it is not restated here, because a second copy is what goes stale. Spec 015's smoke test surfaced that `/ductus` actually invokes more Bash commands than this list covers — observed prompts during a routine bootstrap run included `Bash(git status *)` (used by Frontmatter Migration's precheck) and `Bash(awk '{print $5, $9}')` (used by the post-scaffolding summary). Adopters accept these prompts and they accumulate in `settings.local.json`, creating per-adopter drift from what the framework would have shipped if the gaps weren't there.
...
Audit the bootstrap allowlist against actual `/ductus` runtime behavior and extend the canonical `settings_template` in both agent registry rows to cover every Bash command pattern a routine bootstrap exercises.

- **Audit pass.** Trace through a full `/ductus` run (greenfield bootstrap of a fresh adopter project) and enumerate every distinct Bash command pattern the procedure invokes. Sources: `framework/bootstrap/ductus.md` body (the Instructions section), plus any sub-procedures it composes (e.g., Frontmatter Migration, post-scaffolding summary, workflow scaffolding).
...
- **No regressions.** Existing entries stay. The merge behavior (Behavior step 2 in `framework/bootstrap/ductus.md`) ADDs missing entries without deduplicating or reordering, so the canonical list grows monotonically.
...
- **Pattern over-broadness.** `Bash(git *)` would cover `git status`, `git diff`, `git push`, `git reset`, etc. `git push` and `git reset --hard` are operations spec 012 (and AGENTS.md) treats as risky/externally-visible. Prefer narrower patterns (`Bash(git status *)`, `Bash(git log *)`, `Bash(git diff *)`) over `Bash(git *)`. The audit captures only what `/ductus` actually invokes; risky git operations are not in `/ductus`'s path, so they shouldn't end up in the list.
