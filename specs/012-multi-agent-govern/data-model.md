...
| `settings_template` | object (JSON) | yes | Bootstrap entries written into `{config_dir}/settings.local.json` if missing. Native to the agent's settings format. Contains only the curl/ls entries needed for `/ductus` itself — not the full permission set. |
