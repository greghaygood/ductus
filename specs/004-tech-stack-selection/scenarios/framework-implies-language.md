...
- The inferred language is still written as a row in the AGENTS.md **Tech Stack** table — the inference suppresses the *question*, not the *data*. The original reason was workflow recommendation (`005-workflows`), which matched registry entries on `backend_language`, so omitting the row would have silently dropped the Ruby-triggered workflows (RuboCop, RSpec). `043-workflows-sunset` removed that mechanism, and the requirement stands on the consumers that remain: the table documents the stack for contributors, and `/ductus:review`'s tech-stack alignment check reads it — which is the rationale `/ductus:init` itself now gives.
