...
The defect is concentrated in one file — `framework/bootstrap/ductus.md` — where MCP
...
(`ductus.md:80-88`): a new `claude-style` agent now also needs an MCP descriptor entry.
The "Adding a new agent" note is updated to say so.

### Two registration mechanisms, keyed by scope

- **`write-file`** (`project-committed`): ductus writes the repo file additively, exactly
...
### State-B branch (`ductus.md` §State B step 1, §MCP wiring, Pre-flight abort)
...
- **Auggie**: `auggie mcp add ductus --command ductus --args "mcp"` (documented subcommand,
  flag form — paste-safe; no embedded JSON to mangle).
- **Antigravity**: edit `~/.gemini/config/mcp_config.json` to add the `ductus` block, then
...
A dedicated task tests the live `agy` CLI: does a `ductus` entry in project-local
...
- **loads** → Antigravity is `write-file` / `project-committed` on `.agents/mcp_config.json`;
  ductus's current behavior is correct, descriptor recorded, no migration.
...
that removes a stale `.agents/mcp_config.json` from adopter repos on the next `/ductus` run.
...
`README.md:186` currently says ductus "writes the per-agent MCP config (`.mcp.json` for
Claude-style agents, `.agents/mcp_config.json` for Antigravity)" — now inaccurate for
Auggie (claude-style but home-level) and over-committed for Antigravity. Rewrite to: ductus
...
(`ductus.md:22` and `framework/commands/{target,status,analyze,implement,audit,specify,plan,ask}.md`)
...
| `framework/bootstrap/ductus.md` | Modify | Remove MCP row from §Derived values layout table; add per-agent MCP registration table; branch State-B step 1 + §MCP wiring + Pre-flight abort by `mechanism`; update "Adding a new agent" note |
...
- **ductus can't confirm the user ran the surfaced command.** Accepted — the next session's
...
- **Cross-spec signposts touch `done` specs 028/029.** This may trip the `done → in-progress`
  back-edge; the recording mechanism (signpost vs. `/ductus:amend`) is settled at implement time
