...
| --- | --- | --- |
| `review:` | `scenarios/*.md`, `data-model.md` | `reviewed-digest` |
| `analyze:` | every `.md` under the feature, `review.md` and `spec.md` included | `analyzed-digest` |

That inversion is deliberate. `review.md` and `spec.md` are `/{project}:review`'s **outputs**, so counting them would stale every review the instant it was recorded; they are `/{project}:analyze`'s **inputs**, and `review-state-drift` asserts on the `review:` block directly, so excluding them there would exempt the edit that most often invalidates an analyze record. `tasks.md` and `plan.md` are outside the review's set for a separate reason — the first is ephemeral by construction (§tasks-phase), the second churns as Affected Files are revised.

**The comparison is of content, not commits, and that replaced a design that produced false blocks.** Both commands read the **working tree**; `reviewed-against` and `analyzed-against` record a **commit**. Those coincide only when the tree is clean, and at the moment either command runs it usually is not — `/{project}:review` has just written `review.md` and the `review:` block, and `mark-task` and `mark-criterion` rewrote `tasks.md` and `spec.md` before that. Diffing the recorded sha therefore blocked records whose run had genuinely read the current content, as soon as that content was committed, and a gate with false positives is the one people route around. Each record now carries a per-path sha256 of what its run read (`spec.md` digested with its own `analyze:` block excised, since the record is written after the subjects are read and a digest covering it could never match). The two shas survive as **provenance**, read for exactly one thing: the mechanical-sweep rename exemption, which genuinely needs two trees and whose candidates are reported rather than dropped when they are unavailable.

Measured before shipping: without the `analyze:`-block exclusion the rule flagged **all 54** of this repo's recorded specs, because the corpus-wide analyze run had written its own block into every one; with it, 1 of 54 flagged, and that one was a true positive.
...
- **The historical roots are the point, not completeness for its own sake.** The orphan a migration chain leaves is a reference to the path *before* the move, so it carries the *old* root — a current-roots-only list is blind to exactly the class this check exists to catch. Measured on the 048 AC10 adopter run: after the generators moved root `scripts/` → `.ductus/scripts/` → `.ductus/scripts/`, the adopter's `AGENTS.md` still named `scripts/gen-spec-deps.sh`, the check reported clean, and the operator found it by reading the file. The pre-042 entries are prefixes (`scripts/gen-`, `scripts/lib/`) rather than the `scripts/` directory because the framework owned those and never the whole directory; a `scripts/` root would flag an adopter's own `scripts/build.sh`.
