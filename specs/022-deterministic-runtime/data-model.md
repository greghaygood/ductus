...
| 2 | `.ductus/config.toml` | `.ductus/session.toml` | [042](../042-consolidate-govern-per-project-files-under-govern-directory/spec.md) |
...
- `analyzed-digest` / `analyzed-unreadable` — taken by the primitive from the feature directory, never passed in. `/{project}:analyze` is read-only, so the subjects are byte-identical to the ones its passes read moments earlier, and deriving the digest means no caller can record one it did not take. It also guarantees this digest and the one `check-review-gate` recomputes come from a single function, which is what makes the row `/{project}:review` renders and the gate's verdict agree. A subject that exists but could not be read — or could not be *reached*, which are separate failures one line apart in the walk — is recorded in `analyzed-unreadable` rather than digested as empty.

`write-review` derives `reviewed-digest` / `reviewed-unreadable` the same way over the narrower contract set.
...
Project-scoped: no feature argument, like `check-orphaned-references` and
...
The deterministic surface behind `/ductus:implement`'s completion gate, which the host previously walked by hand on every completion attempt. Evaluates every check in documented order, first failure wins. `blocked-by` names the failing one:

0. **`already-done`** — the spec is at `status: done`, so the `in-progress → done` transition this gate authorizes has already happened and no check below it can matter. Deliberately **not** `passed: true`: a gate reporting a pass for a spec it did not examine is the `QUAL-CLAIM-001` conflation the rest of the list exists to prevent, and a caller could read it as authorization to transition a spec that is already transitioned. Ordered first so a completed spec is never held against a lint violation introduced after it closed — and because the freshness checks below would answer *wrongly* here rather than pointlessly: the completing `set-status` rewrites `spec.md`, one of the analyze record's own subjects. Only the exact value `done` short-circuits; an unrecognized status falls through, since `validate-frontmatter` owns reporting a bad value (scenario `a-done-spec-has-no-transition-to-gate`).
...
5. **`not-reviewed`** (`review.md` absent, or its `last-run` missing or null) and 6. **`must-violations`** (`blocking: true`) — the review record, read from `review.md` (spec 057; a `review:` block in `spec.md` frontmatter before it). A `review.md` that exists but carries no parseable record is a third state, **undeterminable**, reported as such rather than as never-reviewed.
...
`cross-spec-impact:` is a hand-authored list of feature slugs a spec owes a change to under §cross-spec-impact ([050](../050-constitution/spec.md), scenario `a-declared-cross-spec-impact-gates-done`). Checks 3 and 4 sit beside each other and ahead of the review record for one shared reason, which check 3 already states: a spec carrying an obligation nobody has discharged is not a candidate for `done`, so whether its review is fresh does not yet matter.
...
Both staleness checks are one computation — `analyze_subjects::freshness_of`, returning `RecordFreshness` (`never-run` / `current` / `stale` / `undeterminable`) — differing only in which files are the subject:

| record | subject set | digest field |
| --- | --- | --- |
| `review.md` | `scenarios/*.md`, `data-model.md` | `reviewed-digest` |
| `analysis.md` | every `.md` under the feature — `review.md` and `spec.md` included, `analysis.md` itself **excluded** | `analyzed-digest` |

That inversion is deliberate. `review.md` and `spec.md` are `/{project}:review`'s **outputs**, so counting them would stale every review the instant it was recorded; they are `/{project}:analyze`'s **inputs**, and `review-state-drift` asserts on `review.md`'s record directly, so excluding them there would exempt the edit that most often invalidates an analyze record. (`spec.md`'s exclusion from the review set rested on `write-review` touching it, which spec 057 stopped; the set is deliberately left unchanged rather than widened, since adding it would stale every review on any spec-body edit.) `tasks.md` and `plan.md` are outside the review's set for a separate reason — the first is ephemeral by construction (§tasks-phase), the second churns as Affected Files are revised.

**The comparison is of content, not commits, and that replaced a design that produced false blocks.** Both commands read the **working tree**; `reviewed-against` and `analyzed-against` record a **commit**. Those coincide only when the tree is clean, and at the moment either command runs it usually is not — `/{project}:review` has just written `review.md` (and, before spec 057 gave that record one home, the spec's `review:` block with it), and `mark-task` and `mark-criterion` rewrote `tasks.md` and `spec.md` before that. Diffing the recorded sha therefore blocked records whose run had genuinely read the current content, as soon as that content was committed, and a gate with false positives is the one people route around. Each record now carries a per-path sha256 of what its run read, with the analyze record's own artifact **excluded from the subject set** — the record is written after the subjects are read, so a digest covering it could never match. That exclusion was block surgery on `spec.md`'s `analyze:` block until spec 057 moved the record; `spec.md` is digested **whole** now, and `analysis.md` is not a subject. Excising only its frontmatter was tried and was insufficient: `write-analysis` rewrites the report body in the same call, so a run whose findings changed staled itself and converged only on a second identical run. The two shas survive as **provenance**, read for exactly one thing: the mechanical-sweep rename exemption, which genuinely needs two trees and whose candidates are reported rather than dropped when they are unavailable.

Measured before shipping, against the `analyze:` block as it then stood: without that exclusion the rule flagged **all 54** of this repo's recorded specs, because the corpus-wide analyze run had written its own block into every one; with it, 1 of 54 flagged, and that one was a true positive.

**A record with no digest is `undeterminable`** — not current, not stale, and not a sha-diff fallback. It does not block, so freshness stops being *enforced* for a spec until its next run writes a digest; it is self-healing rather than a hole, and deliberately not a grandfather clause — the record is not exempt, it is unreadable, and the passing verdict's `guidance` says which. `reviewed_digest` is an `Option` for a case the analyze side cannot have: a spec with no scenarios and no data model records an **empty** digest, which is current, and `Some({})` is the only way to distinguish that from `None`. `spec.md` is always an analyze subject, so an empty analyze digest cannot occur.

A **passing** gate therefore still carries `guidance` when either record is undeterminable. There is no longer a notice about contracts the gate could not examine: that existed because the check compared committed trees and could not see a contract living only in the working tree, and the digest reads the tree directly. A check that can no longer fire is indistinguishable from one that passed, so it was removed rather than kept (scenario `the-committed-tree-horizon`).
...
- **Managed roots** are `.ductus/`, `.githooks/`, the configured spec root (read from config, not assumed), and the **historical** roots `.ductus/`, `scripts/gen-`, and `scripts/lib/`. A broken link into the adopter's own `docs/` is their business; scoping the check is what keeps it from being noise.
...
`check-review-agreement` read them and reported two kinds, because the repairs differ — `examined-nothing` (`examined: 0` over a non-empty `scope`; the record says the passes read nothing) and `examined-unstated` (no `examined` on a record carrying a `scope`). Spec 057 retired that primitive with the second copy of the record it reconciled, so **nothing reads these two fields now**; they are read at the completion gate by a person. The loss is real and was accepted with the relocation: the check compared two sides one `write-review` call had written from the same values, so it never compared two independent observations.
