...
- adopter projects on an older shipped `gen-spec-deps.sh` (the script ships with `update` strategy, but adopters who have pinned it in `.ductus/config.toml` `pinned.files` or have not yet re-run `/ductus` since the cycle-detection fix landed are still on the older copy);
