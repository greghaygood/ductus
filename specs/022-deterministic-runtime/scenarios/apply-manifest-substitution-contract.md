...
- **Is adding fields to the result a breaking change for hosts?** No. Both aggregate fields are additive, and the per-entry field is `Option` with `skip_serializing_if`, so an existing consumer sees the same JSON it saw before for every entry that did not substitute and one extra key for those that did. The installer's (`framework/bootstrap/ductus.md`) §Post-Scaffolding Output now surfaces both numbers, since a diagnostic nobody prints is one nobody reads.
