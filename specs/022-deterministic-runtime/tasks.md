...
- [x] Implement the behavior described in `scenarios/derive-references-unstaged-drift-is-reported.md`
...
- [x] Exclude a `§` inside an inline code span — notation described, not cited — reusing the shared `inline_code_spans` helper rather than a second scanner
...
## 109. Reconcile 022's acceptance criteria with tasks 107 and 108
...
## 116. Sibling-link grammar delegates to the shared feature-directory rule

- [x] Implement the behavior described in `scenarios/sibling-link-grammar-is-the-shared-one.md`

- **Done when**: `derive_dependencies::leading_slug` and `derive_references::is_spec_slug` both delegate to `is_feature_slug`/`parse_feature_dir` rather than re-deriving the digit rule; a body link to a branch-scoped (`1234.1-slug`) or four-digit (`1000-slug`) sibling harvests an edge; a link to a non-feature directory still harvests none; and both behaviours are pinned by tests in `derive_dependencies`'s suite. Verified by probe in both directions, not by reading.

## 117. An empty-scope flag no longer decides the review denominator

- [x] Implement the behavior described in `scenarios/an-empty-scope-flag-does-not-decide-the-denominator.md`
...
- [x] Sweep the corpus with both binaries and record the measured effect in the scenario — findings before and after, and the criteria that match for a reason other than a path's creation — rather than claiming precision from the shape of the change
...
- [x] Implement the behavior described in `scenarios/severity-is-a-closed-set-not-a-string.md`
- [x] Bind the two vocabularies as separate types — review (`must`/`should`) and analyze (`hard-fail`/`blocking`/`advisory`/`informational`) — serializing to the strings they carry today so `review.md` and `analysis.md` stay byte-identical and no consumer's parsing moves
- [x] Make an out-of-set value a deserialization **error** naming the value and the field, never a default — the defect is not that a bad value exists but that it currently resolves to the permissive tier
- [x] Remove `write_review`'s `else`-is-`should` catch-all (`:118-128`) and `finding_rank`'s matching string test (`:402`) by branching on the bound value, so a non-`must` severity can no longer silently write `blocking: false` past `check-review-gate`
- [x] Replace the inline literals at every site — 16 in `check_artifacts.rs`, 15 in `validate_frontmatter.rs`, 3 in `schema/extensions.rs`, 2 in `schema/primitives.rs`
- [x] Decide and state the case-insensitivity contract: both current comparisons use `eq_ignore_ascii_case`, so preserving or narrowing it is a decision to record rather than fall into
- [x] Sweep the corpus for any existing `review.md` / `analysis.md` carrying an out-of-set severity before landing, so a loud failure surfaces here rather than at a later gate
- [x] Record the two closed sets in 022's `data-model.md`, pointing at §text-first-artifacts for the analyze tiers' meaning rather than restating the assignment
- [x] Prove the permissive-default fix by probe in both directions: a well-formed `must` still blocks, and a mis-spelled severity is now rejected rather than filed as a SHOULD
...
## 123. The pi command candidate

- [x] Implement the behavior described in `scenarios/the-pi-command-candidate.md`
- [x] Add {cli-config-dir}/prompts/{project}-{command_name}.md as the last candidate of Host::command_file_candidates (058), keeping the plural-then-singular order of the two pre-existing shapes
...
- [ ] Record the three-shape resolution order in 022's data-model.md under Per-project file resolution
