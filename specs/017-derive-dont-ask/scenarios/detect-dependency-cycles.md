...
- The check is unconditional in the ductus repo (CI catches drift) and in adopter projects (the shipped pre-commit hook propagates the failure).
...
- **Q1: Cycle-check failure mode in the existing pre-commit hook.** Confirmed during implementation: no hook edit required. Both the ductus-repo hook (`.githooks/pre-commit`) and the shipped adopter hooks (`framework/bootstrap/hooks/pre-commit` wrapper and `framework/bootstrap/hooks/ductus-pre-commit` managed inner) all run under `set -euo pipefail`, so a non-zero exit from `scripts/gen-spec-deps.sh` propagates unswallowed and blocks the commit. Naming note: the scenario originally referred to `.githooks/ductus-pre-commit`; the ductus repo actually uses a single `.githooks/pre-commit` (the `ductus-pre-commit` split exists only on the adopter side, where the outer `pre-commit` wraps the managed `ductus-pre-commit`).
