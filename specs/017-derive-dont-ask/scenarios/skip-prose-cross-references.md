...
- **Adopter projects on an older shipped `gen-spec-deps.sh`** (the script ships to adopters with `update` strategy per AC23 — every `/ductus` run refreshes it from upstream): adopters pick up the opt-out automatically on the next `/ductus` run. Adopters who have pinned the script in `.ductus/config.toml` `pinned.files` keep their existing copy and see no behavior change until they unpin or hand-merge.
