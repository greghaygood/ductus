...
**The subject set.** A spec's analyze record covers every `.md` under `specs/{feature}/` — `spec.md`, `plan.md`, `tasks.md`, `data-model.md`, `scenarios/*.md`, `review.md`, and `analysis.md` itself. Analyze's families read all of them, so all of them are its subject. Two exclusions: the run's **own record**, because it is written *after* the subjects are read and counting it would make every run stale itself; and a change that is a uniform §spec-lifecycle rename, which says what it said before, matching the review check's identical carve-out.

The first exclusion moved with the record. When this scenario was written the record was a `analyze:` block in `spec.md` frontmatter, so the exclusion was block surgery on a subject that stayed in the set. `057-analyze-artifact-and-record-relocation` made the record `analysis.md`'s frontmatter, so the excision applies there and **`spec.md` is digested whole** — the block-surgery path for it is gone rather than re-pointed. The reason is unchanged, and it is the reason that carried across: a record written after its subjects are read can never digest itself.
