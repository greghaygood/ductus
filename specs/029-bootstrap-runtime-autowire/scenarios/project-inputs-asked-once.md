...
- **Read back; prompt only for what is missing.** Input collection moves to a new **§Collect Project Inputs** step that runs *after* the Pre-flight Phase (past its abort point). It resolves each input from the first available source — `$ARGUMENTS`, then `.ductus/config.toml`'s `[project]` table (`[host] project` as a fallback for configs predating `[project]`), then an interactive prompt — and prompts only for what none of those supply.
...
- **`.ductus/config.toml` absent or `[project]` missing.** Treated as "not yet collected" — prompt, then write. A malformed `.ductus/config.toml` still aborts per §Project Configuration before this step relies on it.
