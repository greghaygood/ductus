...
The wiring file is the per-layout path already named in the bootstrap's §Derived values: `.mcp.json` at the repo root for the `claude-style` layout, `{config_dir}/mcp_config.json` for the `antigravity` layout. The entry registers the server as `ductus` running the runtime's MCP subcommand.
...
- [x] AC8: The gate wires `ductus` on presence of the binary alone and performs no version-compatibility check (deferred to the runtime per spec 022 §Versioning enforcement).
