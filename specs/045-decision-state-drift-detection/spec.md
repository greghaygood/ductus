...
Repo `ductus`, spec `026-framework-self-audit`, 2026-08-02, found while running that spec's completion gate. Two acceptance criteria were checked `[x]` and had been true when 026 reached `done` in May:
...
- [x] AC1: §drift-prevention names the decision trigger, not only the document-edit trigger, and lists the recognizable resolution events
- [x] AC2: §drift-prevention states that a resolution is incomplete while a sibling artifact still describes the prior state
- [x] AC3: `/{project}:analyze` emits an advisory finding when an artifact's link-adjacent prose asserts an open state that the link target's current state contradicts
- [x] AC4: Each finding names the citing file and line, the link target, and the target's contradicting state
- [x] AC5: A feature directory whose prose matches its link targets' state produces zero findings
- [x] AC6: The link-adjacent check covers every artifact in a feature directory that carries inline sibling links — `spec.md`, `plan.md`, `tasks.md`, and `scenarios/*.md`
- [x] AC7: Findings from both checks are advisory (non-blocking) at introduction and do not gate `done`
- [x] AC8: Repeat runs over an unchanged feature directory produce identical findings
- [x] AC9: A link whose target state cannot be read produces no finding (an unknown is never escalated to a defect, matching the `status-unreadable` precedent in cross-service reference classification)
- [x] AC10: `analyze.md` documents both checks and their shared promotion criterion alongside the existing advisory checks
- [x] AC11: The tell list is exactly the six closed entries, framework-fixed, with no per-project configuration surface
- [x] AC12: The scanned unit is the enclosing block-level element (list item, table row, or paragraph), and evaluation is per link so a multi-link block fires only for the contradicting target
- [x] AC13: A tell inside a fenced code block, an HTML comment, a blockquote, or an inline code span produces no finding
- [x] AC14: A link targeting a scenario is evaluated on the scenario's open-question count and file existence; a tell requiring a lifecycle status produces no finding
...
- **Should a removal-phrased criterion be checked by inverting the assertion, rather than skipped?** No — measured and rejected on 2026-08-30. The premise is sound and the arithmetic is not. A criterion claiming a path is *absent* is as checkable as one claiming presence, so `not-a-live-claim` looked like a gap: 72 criteria across 18 specs had nothing verifying their substance, and a hand-walk of all 72 found two stale claims (003's AC2 and 025's AC3). But the claim is not the hard part — **attribution** is. Of the 31 removal-marked criteria, only **5 name a single path**; the other 26 name two or more and make *opposite* claims about them in one sentence ("`install.sh` is deleted; its actions are inlined into `ductus.md`"). A clause-scoped implementation split on `;` and the em dash was built and run over the corpus: 20 findings, of which roughly 8 were false — `023`'s AC3 reported `framework/constitution.md` as wrongly present because the word *deleted* referred to two command verbs that spec 023 had retired, not to the file. It also **broke the existing remedy**: an annotation is appended at the end of a criterion, so clause scoping stopped it qualifying the earlier clause, and six previously-suppressed findings returned. Deciding which path a phrase is about is semantic judgment, which §runtime-boundary places at an extension point rather than in a deterministic family — and a family that fires falsely on a true criterion is worse than the silence it replaces, the same standard that rejected the criterion-supersession check at 455 pairs. The remedy stays the completion-gate hand-walk `AGENTS.md` prescribes. What the measurement changes is that its cost is now known: 72 criteria, about 3% carrying drift, and a checkable subset of 5 too small to justify a second marker taxonomy and a four-place parity update.
