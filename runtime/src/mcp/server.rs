...
impl GovRuntimeServer {
    ...
    #[tool(
        name = "retire-feature",
        description = "Remove a branch-scoped feature directory (`<identifier>.<n>-<slug>`) once its content has been folded into the upstream spec that receives it — the last step of a fold and the only irreversible one. This is where the fold target's existence is finally enforced: everything earlier checks `folds-into` for shape only, because a branch-scoped spec exists precisely because upstream diverged, so before the merge its target normally lives on the upstream branch. Fold-back runs after the merge, in the first tree holding both, which is the first moment the question has an answer worth refusing on. Refuses when `fold-target` names no feature directory holding a spec.md (a directory with no spec is not a home content can have landed in), and refuses a sequential `NNN-slug` feature unless `allow-sequential` is set — the sequential form is permanent, and the refusal is gated rather than removed so that an irreversible operation stays out of reach of a typo: fold never sets the flag, so a mistyped feature name on a fold still refuses, while consolidation sets it having already named both specs and confirmed the removal. The spec.md guard above is **not** gated — opting in to removing a sequential directory is not opting in to stranding its content. `retired: false` is the domain outcome for a directory already gone, so re-running an interrupted fold converges rather than halting."
    )]
