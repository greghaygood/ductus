...
A minimal fixture for the `/ductus:target` parity test. The status is
`planned` so the host-side render after the runtime returns will route
to `/ductus:implement` as the next pipeline step.

## Motivation

Exercises the runtime's `read-spec` primitive when /ductus:target is
